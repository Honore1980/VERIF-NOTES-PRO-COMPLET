...
function generateSecretCode(matricule, note) {
  // LOGIQUE SECRÈTE BURKINA - Seul vous la connaissez
  const SECRET_SALT = "AES-BF-DDG-2026";
  const data = `${matricule.trim().toUpperCase()}#${note}#${SECRET_SALT}`;

  // 1. Calcul d'un checksum simple mais obscur
  let sum = 0;
  for(let c of data) sum += c.charCodeAt(0) * 7;

  // 2. Code = Base64 + checksum + inversion
  const raw = `${matricule}-${note}-${sum.toString(36).toUpperCase()}`;
...
function verifySecretCode(code) {
  try {
    ...
    const b64_rev = parts[0];
    const b64 = b64_rev.split('').reverse().join('');
    const raw = atob(b64);
    const [matricule, note, hash] = raw.split('-');
